...
export function VibeCard({ card, onAction, zIndex }) {
  ...
  return (
    ...
    >
      <div className="relative w-full h-80 mb-6 rounded-[32px] overflow-hidden">
        {hasImage ? (
          <img src={card.image_url} alt={card.word} className="w-full h-full object-cover shadow-2xl pointer-events-none" />
        ) : (
          <div className="placeholder-grad pointer-events-none">{card.word[0]}</div>
        )}
      </div>

      <div className="text-center pb-4 select-none">
        <div className="flex items-center justify-center gap-3 mb-2">
          <h2 className="text-4xl sm:text-5xl font-black text-white tracking-tighter">{card.word}</h2>
          <button 
            onClick={() => speak(card.word)}
            className={`p-2.5 rounded-full transition-all duration-300 shadow-lg ${isSpeaking ? 'text-indigo-300 bg-indigo-500/30 scale-110' : 'text-slate-300 bg-white/5 hover:bg-white/15'}`}
          >
            <Volume2 size={24} />
          </button>
        </div>
